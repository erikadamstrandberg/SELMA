...
from vcsel_chip_shapes import create_initial_a_mark, create_EBL_combo_mark, create_label
from initialize_layer_data import create_layer_data
from manage_gdstk import setup_gds_lib, save_gds_file, create_folder_for_mask, save_ordering_mask, save_single_layer, save_single_layer_inverted, save_single_layer_matrix, save_single_layer_matrix_inverted
...
def create_chip_necessities(chip_mask):
    ...
    combo_mark_top_right2 = create_EBL_combo_mark('MS_align', x_offset_2_combo_mark, c, chip_mask.layer_data)
    ...
    ## EBLMARKS
    EBL_mark_size = 20
    EBL_UR_x = frame_size_x/2 + 100
    EBL_UR_y = frame_size_y/2 - aligment_mark_squares
    chip_mask.create_rectangle('p_ring', EBL_UR_x, EBL_UR_y, EBL_mark_size, EBL_mark_size)
    
    EBL_UL_x = -frame_size_x/2 - 100
    EBL_UL_y = frame_size_y/2 - aligment_mark_squares
    chip_mask.create_rectangle('p_ring', EBL_UL_x, EBL_UL_y, EBL_mark_size, EBL_mark_size)
    
    EBL_LR_x = frame_size_x/2 + 100
    EBL_LR_y = -frame_size_y/2 + aligment_mark_squares
    chip_mask.create_rectangle('p_ring', EBL_LR_x, EBL_LR_y, EBL_mark_size, EBL_mark_size)
    
    EBL_LL_x = -frame_size_x/2 - 100
    EBL_LL_y = -frame_size_y/2 + aligment_mark_squares
    chip_mask.create_rectangle('p_ring', EBL_LL_x, EBL_LL_y, EBL_mark_size, EBL_mark_size)
...
def main():
    ...
    # Mask names
    mask_name   = 'KAW_BE_v2_MLA_top_alignment'
    # Name of top cell
    cell_name   = 'KAW_BE_v2_MLA_top_alignment'
    ...
    # for i in range(number_of_VCSEL_columns_left):
    #     for j in range(number_of_VCSEL_rows_left):
    #         VCSEL_y_trenches(x_offset_0_1 + x_offset_between_VCSEL*i, y_offset_0_1 - y_offset_between_VCSEL*j, mesa_radius[j], mesa_to_trench[i], chip_mask, layer_data)
    
    # ## 1.1 qudrant
    # for i in range(number_of_VCSEL_columns_right):
    #     for j in range(number_of_VCSEL_rows_right):
    #         VCSEL_x_trenches(x_offset_1_1 + x_offset_between_VCSEL*i, y_offset_1_1 - y_offset_between_VCSEL*j, mesa_radius[j], mesa_to_trench[i], chip_mask, layer_data)
    ...
    matrix_dict = {'rows' : np.array([1, 3, 4, 4, 4, 3, 1]),
                   'translation_x' : 8000.0,
                   'translation_y' : 10000.0,
                   'wafer_size_inches' : 1.5}

    ## Create dict for backside alignment marks
    alignment_marks_for_backside = {}
    alignment_marks_for_backside['left_side']  = []
    alignment_marks_for_backside['right_side'] = []
    alignment_marks_for_backside['upper_side']  = []
    alignment_marks_for_backside['lower_side'] = []
    alignment_marks_for_backside['numbering'] = {}
    
    alignment_marks_for_backside['numbering'][-4] = []
    alignment_marks_for_backside['numbering'][4] = []
    alignment_marks_for_backside['numbering'][0] = []
    alignment_marks_for_backside['numbering'][6] = []
    
    
    ## Create alignment marks on the left side
    alignment_mark_upper = create_EBL_combo_mark('p_ring', 3500, 3000, layer_data)
    alignment_mark_lower = create_EBL_combo_mark('p_ring', 3500, -3000, layer_data)
    alignment_marks_for_backside['left_side'].append(alignment_mark_upper)
    alignment_marks_for_backside['left_side'].append(alignment_mark_lower)
    
    # Numbers on left side
    label_1 = create_label('p_ring', 3463, 3500, 150, '4', layer_data)
    alignment_marks_for_backside['numbering'][-4].append(label_1)
    label_7 = create_label('p_ring', 3463, -2500, 150, '5', layer_data)
    alignment_marks_for_backside['numbering'][-4].append(label_7)
    
    ## Create alignment marks on the right side
    alignment_mark_upper = create_EBL_combo_mark('p_ring', -3500, 3000, layer_data)
    alignment_mark_lower = create_EBL_combo_mark('p_ring', -3500, -3000, layer_data)
    alignment_marks_for_backside['right_side'].append(alignment_mark_upper)
    alignment_marks_for_backside['right_side'].append(alignment_mark_lower)

    # Numbers on right side
    label_6 = create_label('p_ring', -3537, 3500, 150, '1', layer_data)    
    alignment_marks_for_backside['numbering'][4].append(label_6)
    label_12 = create_label('p_ring', -3537, -2500, 150, '8', layer_data)    
    alignment_marks_for_backside['numbering'][4].append(label_12)
    
    ## Create alignment marks on upper part
    alignment_mark_upper = create_EBL_combo_mark('p_ring', 3500, -3000, layer_data)
    alignment_mark_lower = create_EBL_combo_mark('p_ring', -3500, -3000, layer_data)
    alignment_marks_for_backside['upper_side'].append(alignment_mark_upper)
    alignment_marks_for_backside['upper_side'].append(alignment_mark_lower)
    
    # Numbers on right side
    label_6 = create_label('p_ring', 3463, -2500, 150, '2', layer_data)    
    alignment_marks_for_backside['numbering'][0].append(label_6)
    label_12 = create_label('p_ring', -3537, -2500, 150, '3', layer_data)    
    alignment_marks_for_backside['numbering'][0].append(label_12)
    
    ## Create alignment marks on lower part
    alignment_mark_upper = create_EBL_combo_mark('p_ring', 3500, 3000, layer_data)
    alignment_mark_lower = create_EBL_combo_mark('p_ring', -3500, 3000, layer_data)
    alignment_marks_for_backside['lower_side'].append(alignment_mark_upper)
    alignment_marks_for_backside['lower_side'].append(alignment_mark_lower)
    
    # Numbers on right side
    label_6 = create_label('p_ring', 3463, 3500, 150, '7', layer_data)    
    alignment_marks_for_backside['numbering'][6].append(label_6)
    label_12 = create_label('p_ring', -3537, 3500, 150, '6', layer_data)    
    alignment_marks_for_backside['numbering'][6].append(label_12)
    
    
    save_single_layer_matrix_inverted(matrix_dict, alignment_marks_for_backside, 'p_ring', chip_mask, mask_name, cell_name, mask_folder, layer_data, chip_size_x, chip_size_y, chip_mask.circle_tolerance)
