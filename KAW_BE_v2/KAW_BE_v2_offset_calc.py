...
def main():
    ...
    # Mask names
    mask_name   = 'KAW_BE_v2_reversed'
    # Name of top cell
    cell_name   = 'KAW_BE_v2_reversed'
    ...
    p_ring_layer = chip_mask.mask_dict['p_ring']
    for i in p_ring_layer:
        if isinstance(i, gdstk.Polygon):
            i.mirror((0, 20000), (0, -20000))
                        
            i.translate(3.5, -7.15)
            i.rotate(-0.15*np.pi/180)
            
    ms_align_layer = chip_mask.mask_dict['MS_align']
    
    for i in ms_align_layer:
        if isinstance(i, gdstk.Polygon):
            i.rotate(np.pi)
            i.translate(0, 100)
    ...
    slits_frame = create_rectangle('slits', 0, 0, chip_size_x, chip_size_y, layer_data)
    
    sits_lower_right = create_rectangle('slits', chip_size_x/4, -chip_size_y/4, chip_size_x/2 - 800, chip_size_y/2, layer_data)
    
    slits_frame = gdstk.boolean(slits_frame, sits_lower_right, 'and',
                            layer=layer_data['slits']['layer_number'],
                            datatype=layer_data['slits']['datatype'])
    
    slits_height = chip_size_x - 1500
    slits_width = np.array([10, 15, 20, 25, 30, 35, 40, 45, 50])
    
    slits_first_pos = -300
    slits_y_offset = 300
    for i in range(len(slits_width)):
        slits_remove = create_rectangle('slits', 0, slits_first_pos - i*slits_y_offset, slits_height, slits_width[i], layer_data)
    
    
        slits_frame = gdstk.boolean(slits_frame, slits_remove, 'not',
                                layer=layer_data['slits']['layer_number'],
                                datatype=layer_data['slits']['datatype'])
        
    
    slits_frame = gdstk.boolean(slits_frame, sits_lower_right, 'xor',
                            layer=layer_data['slits']['layer_number'],
                            datatype=layer_data['slits']['datatype'])
        
    # slits_second_pos = 600
    # for i in range(len(slits_width)):
    #     slits_remove = create_rectangle('slits', slits_second_pos + i*slits_x_offset, 0, slits_width[i], slits_height, layer_data)
    ...
    for i in slits_frame:
        i.translate(2, -12)
        i.rotate(0.00261)
    
    for i in slits_frame:
        chip_mask.mask_dict['slits'].append(i)
