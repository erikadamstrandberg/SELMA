...
x_offset = 260
current_phase_array = np.array([1, 1.5, 2, 2.5, 3, 3.5, 4, 4.5, 5, 5.5, 6, 0])

for k, current_phase in enumerate(current_phase_array):
            ...
            if grid[i][j] == 1:
                if first:
                    CGAN_filled = create_rectangle(
                        x[i] + x_pos + x_offset*k, y[j] + y_pos, x_size, y_size, 1)
                    first = False
                else:
                      CGAN_filled = gdstk.boolean(CGAN_filled, create_rectangle(
                                                               x[i] + x_pos + x_offset*k, y[j] + y_pos, x_size, y_size, 1), 'or')
