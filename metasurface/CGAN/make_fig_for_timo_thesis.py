...
f = 100
phase_map = phase_profile_lens_convex(980e1, f, R)
...
for ii in range(len(phase_array)):
    current_phase = phase_array[ii]
    ...
    for i in range(len(x)):
        for j in range(len(y)):
            if grid[i][j] == 1:
                rect1 = mpl.patches.Rectangle((x[i] + x_pos + ii*grid_period, y[j] + y_pos), pixel_size, pixel_size, fc='black')
                ax.add_patch(rect1)
                
            
            
plt.xlim([0, grid_period*len(phase_array)])
plt.ylim([0, 260])
plt.axis('equal')
